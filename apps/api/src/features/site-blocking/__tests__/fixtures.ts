...
// contracts' `z.string()` createdAt field the moment a test asserts against
// it. Disabling it keeps what these tests see equal to the real wire format
...
export const VALID_USER: RegisterRequest = {
	email: 'site-blocking@example.com',
	name: 'Site Blocking User',
	password: 'password123',
}

// A second, distinct user — needed by tests that check that one user's
// blocked sites (and mutations against them) are isolated from another's.
export const OTHER_USER: RegisterRequest = {
	email: 'site-blocking-other@example.com',
	name: 'Other Site Blocking User',
	password: 'password123',
}
