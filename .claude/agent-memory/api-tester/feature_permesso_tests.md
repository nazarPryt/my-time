...
- `POST /permesso/reset` ("danger zone" full wipe/unsubscribe) — 401 unauthenticated; full wipe returns `toStatusResponse(null)` empty shape after practice number + schedule + a seeded history row exist; `GET /permesso` and `GET /permesso/history` both come back empty afterward and the subscription row is verified gone via direct DB read (not just filtered from the response); no-op case when the user never had a subscription row at all (still 200, still empty shape). History rows for the reset test are seeded via direct `db.insert(permessoChecks, ...)` rather than calling `POST /permesso/check`, since that route hits the real government portal (`checker.ts`) — not something a test should depend on.
