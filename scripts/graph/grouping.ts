...
export function nodeForFile(filePath: string, root: string): GraphNode | null {
  ...
  if (webFeature) {
    ...
    return { id: `web/${featureName}`, label: featureName, app: "web", type: "feature" }
  }
  ...
  if (apiFeature) {
    ...
    return { id: `api/${featureName}`, label: featureName, app: "api", type: "feature" }
  }
  ...
  if (extFeature) {
    ...
    return { id: `extension/${featureName}`, label: featureName, app: "extension", type: "feature" }
  }
  ...
  if (extOther) {
    ...
    return { id: `extension/${dirName}`, label: dirName, app: "extension", type: "feature" }
  }
  ...
  if (contractsFeature) {
    ...
    return { id: `contracts/${featureName}`, label: featureName, app: null, type: "package" }
  }
  ...
  if (pkg) {
    const [, pkgName] = pkg
    return { id: `packages/${pkgName}`, label: pkgName, app: null, type: "package" }
  }
...
export function fileNodeForFile(filePath: string, root: string): GraphNode | null {
  const rel = path.relative(root, filePath).replace(/\\/g, "/")
