...
export function createWorkoutStore(workoutApi: WorkoutApiActions) {
	return create<WorkoutState>((set, get) => ({
		...
		addSet: async (reps) => {
			...
			const { data: created, error: err } = await workoutApi.addSet(
				exerciseType,
				reps,
			)
			if (err || !created) {
				set((s) => ({
					...
					data: s.data
						? {
								...s.data,
								sets: s.data.sets.filter((item) => item.id !== tempId),
								total: s.data.total - reps,
							}
						: s.data,
				}))
		...
		deleteSet: async (id) => {
			...
			set((s) => ({
				data: s.data
					? {
							...s.data,
							sets: s.data.sets.filter((item) => item.id !== id),
		...
		updateGoal: async (targetReps) => {
			...
			const { error: err } = await workoutApi.updateGoal(
				exerciseType,
				targetReps,
			)
			if (err) {
		...
		loadProgress: async (year, month) => {
			...
			const { data: res } = await workoutApi.fetchWorkoutProgress(
				get().exerciseType,
				year,
				month,
			)
